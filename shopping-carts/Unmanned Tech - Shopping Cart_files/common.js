...
//	setProductThumbHeight();
	...
	if ($('body').hasClass('Homepage') == false) {
		$(className + ' .ProductList:not(.List)').not('.masonry').imagesLoaded().always(function(){
	...
	modalOptions = $.extend({
		...
		onShow: function() {
			...
			if (itemTxt) {
				...
				$('.cartLink .total strong').html(itemTotal); 
			}
			setProductListHeights(null, '.fastCartContent');
			$('.fastCartContent .ProductList:not(.List) li').width(ThumbImageWidth);
			
			if ($('#fastCartSuggestive .ProductActionAdd:visible').size() != 0) {
				
				$('#fastCartSuggestive').addClass('ProductActionAddEnabled');	
				
			}; 
			var maxHeight2 = 0;

			$('#fastCartSuggestive  .border').not('.Feat').each(function(){
				if ($(this).height() > maxHeight2) { maxHeight2 = $(this).height(); }
			});
			$('#fastCartSuggestive  .border').not('.Feat').height(maxHeight2); 
		},
		onClose: function() {
